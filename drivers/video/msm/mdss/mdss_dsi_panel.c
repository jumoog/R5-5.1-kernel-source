...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		else if (!pinfo->cont_splash_enabled) {
			if (gpio_is_valid(ctrl_pdata->disp_en_gpio))
				gpio_set_value((ctrl_pdata->disp_en_gpio), 1);
...
/* Xiaori.Yuan@Mobile Phone Software Dept.Driver, 2015/04/10  Add for esd check */
	if(MSM_BOOT_MODE__NORMAL!=get_boot_mode() && is_project(OPPO_14005)){
		pinfo->esd_check_enabled = false;
	}
#endif /*VENDOR_EDIT*/
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if(is_project(OPPO_14005)){ //lile@EXP.BasicDrv.LCD, 2015-11-18, changed for R5 LCD orientation
		pinfo->panel_orientation = MDP_ROT_180;
	}
	...
	//huqiao@EXP.BasicDrv.Basic 2016/3/16 add for R5 lcd flash
